...
export class ContiRicaviCanoniController extends BaseController {
    ...
    public async getAll(options: Record<string, any> = {}): Promise<ContoRicaviCanoni[]> {
        const result = await this.axiosInstance.get(`${this.route}`, { ...options });
        return result.data;
    }

    public async get(id: number, options: Record<string, any> = {}): Promise<ContoRicaviCanoni> {
        const result = await this.axiosInstance.get(`${this.route}/${id}`, { ...options });
        return result.data;
    }

    public async getByCodice(codice: string, options: Record<string, any> = {}): Promise<ContoRicaviCanoni> {
        const result = await this.axiosInstance.get(`${this.route}/codice/${codice}`, { ...options });
        return result.data;
    }

    public async getBySigla(sigla: string, options: Record<string, any> = {}): Promise<ContoRicaviCanoni> {
        const result = await this.axiosInstance.get(`${this.route}/sigla/${sigla}`, { ...options });
        return result.data;
    }

    public async create(body: ContiRicaviCanoniCreateBody, options: Record<string, any> = {}): Promise<number> {
    ...
    public async replace(
        id: number,
        body: ContiRicaviCanoniReplaceBody,
